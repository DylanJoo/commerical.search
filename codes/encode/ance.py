...
class AnceDocumentEncoder(DocumentEncoder):
    ...
    def encode(self, texts=None, titles=None, descriptions=None, max_length=256, **kwargs):
        if texts is None:
            if descriptions is None:
                texts = [title.strip() for title in titles]
            else:
                texts = [f'{title} {description}'.strip() for title, description in zip(titles, descriptions)]
